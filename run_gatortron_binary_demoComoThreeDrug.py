...
class GatorTron_Dataset(Dataset):
    def __init__(self, df, tokenizer, max_len, with_date):
        ...
        # Convert audit.c.score to binary labels
        self.labels = [
            1 if (row['gender'] == 'Man' and row['audit.c.score'] >= 4) or
                 (row['gender'] == 'Woman' and row['audit.c.score'] >= 3)
            else 0
            for _, row in self.df.iterrows()
        ]
...
class GatorTron_BinaryClassifier(BertPreTrainedModel):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Train a GatorTron binary classification model for AUDIT-C Scoring."
    )
    parser.add_argument(
        "--non_linear_head",
        action='store_true',
        default=False,
        help="whether to use non-linear classification head"
    )
    parser.add_argument(
        "--with_date",
        action='store_true',
    ...
    parser.add_argument(
        "--run_name",
        help="like 'gatrotron_binary_demo_como_threeDrug_linear_head'"
    )
    ...
    # init model
    config = AutoConfig.from_pretrained(MODEL_NAME, num_labels=1)  # for binary classification
    model = GatorTron_BinaryClassifier(MODEL_NAME, args.non_linear_head).to(device)
