...
from utils import (DEMO_COMO_THREE_DRUG_PARQUET_PATH, PROJECT_NAME, SEED,
                   compute_metrics_fine_grained, expand_comorbidity,
...
class GatorTron_Dataset(Dataset):
    def __init__(self, df, tokenizer, max_len, with_date):
        ...
        q1_scores = self.df["q1.score"].tolist()
        q2_scores = self.df["q2.score"].tolist()
        q3_scores = self.df["q3.score"].tolist()
        self.labels = [[int(q1), int(q2), int(q3)] for q1, q2, q3 in zip(q1_scores, q2_scores, q3_scores)]
...
if __name__ == "__main__":
    ...
    trainer = Trainer(
        ...
        compute_metrics=compute_metrics_fine_grained,
